...
搭建环境是比较麻烦的事情，需要记下来，后续考虑脚本自动化

## 系统组件 2021-06-19

1. mysql/postges 数据存储及应用开发
2. clickhouse 面向统计分析的数仓服务
3. redis 实时计算数据的存储组件（可以把计算结果阶段性同步到数仓）
4. kafka connect 数据实时采集
5. kafka/pulsar 面向高流量的消息队列
6. spark 提供计算服务（开发环境可考虑本地模拟）
7. 
