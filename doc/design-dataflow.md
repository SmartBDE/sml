...
如果数据被多个应用使用，则需要把数据沉淀到数据仓库。此时数据仓库类似数据湖，
区别于传统的数据仓库建设方式，数据是需要先沉淀而并未先处理，以保证数据的大而全。

不使用二次处理结果的数据原因，是为了实现数据的原貌与交换，此时数据仓库会是更好的选择
数据仓库面临着如何提供实时的处理结果给客户的需求，包括查询的实时反馈，和信息的实时推送

考虑使用metadata实现数据的汇集，数据版本控制/需要字段...

能够通过数据二次处理后，进入到数据仓库的数据，才可以被使用，数据仓库面临着
如何提供实时的处理结果给客户的需求，包括查询的实时反馈，和关注信息的实时推送
能够通过数据二次处理的数据，才可以被应用使用。数据的二次处理与应用需求绑定，

我们可以把数据分为两类，一种是状态数据，一种是行为数据（操作数据），进入到
数据仓库中的数据，此时的建模和计算，都是基于数据仓库，数据仓库还有一个特点，
来自于远端的数据，进入数据仓库后，不再可变
我们可以把数据分为两类，一种是状态数据，一种是行为数据（操作数据）。数据进入
数据仓库后，为数据应用提供数据基础服务，此时应用的数据二次处理、建模和计算，都是基于数据仓库。

数据仓库还有一个特点，来自于远端的数据，进入数据仓库后，不再可变。
如何解决状态数据在数据仓库的保存？状态数据可以采用插入的方式，查询时，只读取
最新的状态信息

最新的状态信息，这是通过数据变化历史实现的。
